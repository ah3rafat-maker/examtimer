...
function shouldApplyForceRelease(settings){
  if (!currentHallLockId || !settings) return false;
  ...
  if (allAt && (!openedAt || allAt >= openedAt)) return true;
  if (oneId && oneId === currentHallLockId && oneAt && (!openedAt || oneAt >= openedAt)) return true;
  return false;
}
...
function renderHallLocksList(docs){
  ...
  box.querySelectorAll("[data-release-lock]").forEach(btn => btn.addEventListener("click", async () => {
    ...
    try {
      ...
      await saveCloudSettings({ lastForceClosedLockId: btn.dataset.releaseLock, lastForceClosedAtMs: stamp });
      setTimeout(() => ref.delete().catch(()=>{}), 8000);
