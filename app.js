...
function getBeepDurationSetting(kind){
  const map = { half: [STORE_KEYS.beepHalfDuration, "beepHalfDuration", 3], fifteen: [STORE_KEYS.beepFifteenDuration, "beepFifteenDuration", 1], end: [STORE_KEYS.beepEndDuration, "beepEndDuration", 10] };
...
function initCloudSync(){
  ...
  try {
    ...
    firestoreDoc(FIRESTORE_DOCS.settings).onSnapshot(snap => {
      ...
      if (!snap.exists) saveCloudSettings({ password: DEFAULT_PASSWORD, supportCode: localStorage.getItem(STORE_KEYS.supportCode) || "2026", attendanceTime: localStorage.getItem(STORE_KEYS.attendanceTime) || "30", supportChairName: localStorage.getItem(STORE_KEYS.supportChairName) || "", supportChairTitle: localStorage.getItem(STORE_KEYS.supportChairTitle) || "", beepHalfDuration: localStorage.getItem(STORE_KEYS.beepHalfDuration) || "3", beepFifteenDuration: localStorage.getItem(STORE_KEYS.beepFifteenDuration) || "1", beepEndDuration: localStorage.getItem(STORE_KEYS.beepEndDuration) || "10", semester: localStorage.getItem(STORE_KEYS.semester) || "ربيع", academicYear: localStorage.getItem(STORE_KEYS.academicYear) || "2025 - 2026", createdAt: Date.now() });
...
function updateTimer(){
  ...
  if (!endHandled && rawRemaining <= 0) {
    ...
    timerEl.textContent = "انتهى الوقت";
    timerEl.classList.add("timer-ended");
    playBeep(getBeepDurationSetting("end"));
    document.body.classList.remove("display-running", "single-course", "multi-course", "many-courses");
    document.getElementById("homeBtn")?.classList.remove("hidden");
    if (document.fullscreenElement) {
...
function initTermSettings(){
  ...
  saveBtn.addEventListener("click", () => {
    ...
    if (!beepHalfDuration || Number(beepHalfDuration) <= 0 || !beepFifteenDuration || Number(beepFifteenDuration) <= 0 || !beepEndDuration || Number(beepEndDuration) <= 0) return alert("يرجى إدخال مدد التنبيه الصوتي بالثواني.");
    ...
    saveCloudSettings({ semester: sem, academicYear: ay, attendanceTime, beepHalfDuration, beepFifteenDuration, beepEndDuration });
    setTitles();
...
function buildSupportReportHtml(requests=[]){
  ...
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>تقرير لجنة دعم ومراقبة الامتحانات</title><style>
    @page{size:A4;margin:14mm} @font-face{font-family:'UTASReport';src:url('assets/fonts/UTAS-Regular.woff2') format('woff2'),url('assets/fonts/UTAS-Regular.ttf') format('truetype');font-weight:400;font-style:normal}body{font-family:'UTASReport',Tahoma,Arial,sans-serif;color:#0B2E6B;line-height:1.55;padding-top:22mm}.print-logo{position:fixed;top:9mm;right:10mm;max-height:42px;max-width:150px;object-fit:contain}.print-header{text-align:center;border-bottom:2px solid #0B2E6B;padding-bottom:10px;margin-bottom:14px}.print-header h1{font-size:23px;margin:4px 0}.print-header h2{font-size:17px;margin:3px 0;color:#123}.print-section h2,h2{font-size:17px;margin:16px 0 8px;color:#0B2E6B}.print-table{width:100%;border-collapse:collapse;margin-bottom:12px;font-size:11px;table-layout:auto}.print-table th,.print-table td{border:1px solid #B8C6D9;padding:5px;text-align:center;vertical-align:middle}.print-table th{background:#EAF2FF}.print-summary-horizontal{font-size:10px}.signature-block{margin-top:36px;text-align:left;color:#111}.report-date{text-align:left;color:#555;font-size:12px;margin-top:8px}@media print{button{display:none}.print-logo{position:fixed}}
