...
async function releaseCurrentHallLock(){
  if (!currentHallLockId || !hallLocksCollection()) { currentHallLockId = ""; currentHallLockKey = ""; return; }
...
async function acquireHallLock(hall, period){
  ...
  try {
    ...
    if (snap.exists) {
      ...
      if (!stale && !sameSession) return { ok:false, data };
    }
    await ref.set({ hall, periodText: hallLockLabel(period), sessionId: currentSessionId, openedAtMs: now, updatedAtMs: now, userAgent: navigator.userAgent || "" });
...
function showHallLockedState(hall, lockData={}){
  ...
  if (empty) {
    empty.classList.remove("hidden");
    empty.innerHTML = `<h2>القاعة ${escapeHtml(hall)} مفتوحة حاليًا على جهاز آخر.</h2><p>الفترة: ${escapeHtml(lockData.periodText || "")}</p><button class="secondary-btn" type="button" onclick="location.reload()">تحديث</button>`;
  }
}
...
function renderHallLocksList(docs){
  ...
  box.querySelectorAll("[data-release-lock]").forEach(btn => btn.addEventListener("click", async () => {
    ...
    try { await hallLocksCollection().doc(btn.dataset.releaseLock).delete(); showAppToast("تم تحرير القاعة بنجاح.", 3000); }
...
function initHallLockAdmin(){
  ...
  document.getElementById("releaseAllHallLocksBtn")?.addEventListener("click", async () => {
    ...
    if (!ok || !hallLocksCollection()) return;
    try { const snap = await hallLocksCollection().get(); await Promise.all(snap.docs.map(d=>d.ref.delete())); showAppToast("تم تحرير جميع القاعات بنجاح.", 3000); }
