...
function hallLockKeyFor(hall, period){ return ""; }
function hallLockLabel(period){ return ""; }
function hallFromLockDoc(row){ return ""; }
function periodFromLockDoc(row){ return ""; }

function stopCurrentHallLockWatcher(){
  if (currentHallLockUnsubscribe) { try { currentHallLockUnsubscribe(); } catch {} currentHallLockUnsubscribe = null; }
  if (hallLockPollTimer) { clearInterval(hallLockPollTimer); hallLockPollTimer = null; }
}
async function checkCurrentHallLockStillOwned(lockId){ return; }
function startCurrentHallLockPolling(lockId){ return; }
...
function handleForcedHallRelease(reason=""){ return; }
function watchCurrentHallLock(lockId){ return; }

async function releaseCurrentHallLock(){
  currentHallLockId = ""; currentHallLockKey = ""; currentHallLockOpenedAt = 0;
  if (hallLockHeartbeatTimer) { clearInterval(hallLockHeartbeatTimer); hallLockHeartbeatTimer = null; }
  stopCurrentHallLockWatcher();
  return;
}
async function heartbeatHallLock(){ return; }
function startHallLockHeartbeat(){ return; }
async function acquireHallLock(hall, period){ return { ok:true }; }
function showHallLockedState(hall, lockData={}){ goHome(false); }
...
function renderHallLocksList(docs){ return; }
function initHallLockAdmin(){ return; }
...
function initAdmin(){
  ...
  document.getElementById("adminSettingsLoginBtn")?.addEventListener("click", () => {
    adminLogout();
    location.hash = "admin";
  });
