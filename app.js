  ...
  hallRequestsUnsubscribe = supportRequestsCollection().where("hall", "==", currentHall).onSnapshot(snap => {
    ...
    snap.docChanges().forEach(change => {
      ...
      if (data.acknowledged && !data.displaySeenAck && Number(data.examEndMs || 0) >= now - 60000) {
        ...
        setTimeout(() => ackModal?.classList.add("hidden"), 3000);
...
function renderSupportPeriodStats(todayExams){
  const box = document.getElementById("supportPeriodStats");
  if (!box) return;
  ...
  const entries = [...periods.entries()].sort((a,b) => {
    const ea = a[1][0], eb = b[1][0];
    return (ea?.period?.startMinutes || 0) - (eb?.period?.startMinutes || 0);
  });
  if (!entries.length) {
    box.innerHTML = `<div class="empty-mini-stat">لا توجد فترات اختبار اليوم</div>`;
    return;
  }
  box.innerHTML = entries.map(([period, exams], idx) => {
    ...
    return `<article class="support-period-card">
      <div class="support-card-title"><strong>${periodName(idx)}</strong><span>${escapeHtml(period)}</span></div>
      <div class="support-mini-metrics">
...
function renderSupportHallStats(todayExams){
  ...
  todayExams.forEach(e => {
    if (!hallMap.has(e.hall)) hallMap.set(e.hall, []);
    hallMap.get(e.hall).push(e);
  });
  const entries = [...hallMap.entries()].sort((a,b) => a[0].localeCompare(b[0], "ar", {numeric:true}));
  if (!entries.length) {
    box.innerHTML = `<div class="empty-mini-stat">لا توجد قاعات مستخدمة اليوم</div>`;
    return;
  }
  box.innerHTML = entries.map(([hall, exams]) => {
    const periods = unique(exams.map(periodKeyForExam));
    return `<article class="support-hall-card">
      <div class="support-card-title"><strong>${escapeHtml(hall)}</strong><span>${toArabicDigits(periods.length)} فترة</span></div>
      <div class="support-mini-metrics">
        <div><strong>${toArabicDigits(uniqueCourseCount(exams))}</strong><span>مقررات</span></div>
        <div><strong>${toArabicDigits(exams.map(e => clean(e.section)).filter(Boolean).length)}</strong><span>لجان/شعب</span></div>
        <div><strong>${toArabicDigits(sumStudents(exams))}</strong><span>طلبة</span></div>
      </div>
    </article>`;
  }).join("");
}
...
function initTermSettings(){
  ...
  saveBtn.addEventListener("click", () => {
    ...
    const payload = { semester: sem, academicYear: ay };
    ...
    [supportCurrentCodeInput, supportNewCodeInput, supportConfirmCodeInput].forEach(el => { if (el) el.value = ""; });
    setTitles();
    alert(payload.supportCode ? "تم حفظ إعدادات الفصل الدراسي وتغيير رمز لجنة الدعم." : "تم حفظ إعدادات الفصل الدراسي.");
  });
}
