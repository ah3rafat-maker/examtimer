const STORE_KEYS = {
  ...
  attendanceMinutes: "finalExamTimer.attendanceMinutes"
};
...
function getAttendanceMinutes(){
  const raw = (cloudSettingsCache && cloudSettingsCache.attendanceMinutes) || localStorage.getItem(STORE_KEYS.attendanceMinutes) || "30";
  const n = Number(raw);
...
function initCloudSync(){
  ...
  try {
    ...
    firestoreDoc(FIRESTORE_DOCS.settings).onSnapshot(snap => {
      ...
      if (!snap.exists) saveCloudSettings({ password: DEFAULT_PASSWORD, supportCode: localStorage.getItem(STORE_KEYS.supportCode) || "2026", attendanceMinutes: localStorage.getItem(STORE_KEYS.attendanceMinutes) || "30", semester: localStorage.getItem(STORE_KEYS.semester) || "ربيع", academicYear: localStorage.getItem(STORE_KEYS.academicYear) || "2025 - 2026", createdAt: Date.now() });
      ...
      if (settings.attendanceMinutes !== undefined) localStorage.setItem(STORE_KEYS.attendanceMinutes, String(settings.attendanceMinutes));
...
function formatSectionLabel(section){
  ...
  const digits = toArabicDigits(raw).replace(/\D/g, "");
  if (!digits) return raw;
  return digits.length > 1 ? `SIS(${digits})` : `CIMS(${digits})`;
}
function buildSupportRequestLabel(req){
  if (req && req.type === "ABSENCE") {
    const count = Number(req.absenceCount || 0);
    const section = req.sectionLabel || formatSectionLabel(req.section || "");
    return `يوجد غياب عدد ${toArabicDigits(count)} طالب في مقرر (${req.courseName || ""} - ${req.courseCode || ""}) شعبة ${section}`;
  }
  const type = req.type || "استدعاء";
  if (type === "استدعاء مدرس المقرر") {
    const reason = req.teacherReason ? ` - ${req.teacherReason}` : "";
    return `${type}${reason} (${req.courseName || ""}${req.courseCode ? " - " + req.courseCode : ""})`;
  }
  ...
  hallRequestsUnsubscribe = supportRequestsCollection().where("hall", "==", currentHall).onSnapshot(snap => {
    ...
    snap.docChanges().forEach(change => {
      ...
      if (data.acknowledged && !data.displaySeenAck && Number(data.examEndMs || 0) >= now - 60000) {
        showAppToast(data.type === "ABSENCE" ? "تم استلام بلاغ الغياب من لجنة الدعم." : "تم استلام طلبك من لجنة الدعم.", 3000);
...
function supportRequestHtml(id, req){
  ...
  return `<article class="support-notification ${req.type === 'ABSENCE' ? 'is-absence' : ''} ${req.acknowledged ? 'is-acknowledged' : ''}">
...
function renderSelectedSupportPeriod(idx){
  ...
  const absenceTotal = absenceTotalForPeriod(period);
  const absenceRate = students ? ((absenceTotal / students) * 100).toFixed(2) : "0.00";
  details.innerHTML = `<div class="support-period-summary compact">
    ...
    <div><strong>${toArabicDigits(absenceTotal)}</strong><span>غياب</span></div>
...
function todayAbsenceRows(){
  return supportAbsenceRows.filter(r => isTodayMs(r.createdAtMs));
}
function absenceTotalForPeriod(periodText){
  return todayAbsenceRows()
    .filter(r => !periodText || r.periodText === periodText)
    .reduce((sum, r) => sum + (Number(r.absenceCount) || 0), 0);
}
function committeeCountForToday(todayExams){
  const set = new Set();
  todayExams.forEach(e => set.add(`${periodKeyForExam(e)}|${e.hall}`));
  return set.size;
}
...
function updateSupportStats(){
  ...
  setStat("supportTodayHalls", committeeCountForToday(todayExams));
  setStat("supportTodayStudents", sumStudents(todayExams));
  setStat("supportTodayAbsence", todayAbsenceRows().reduce((s,r)=>s+(Number(r.absenceCount)||0),0));
...
function attachSupportRequestsListener(){
  ...
  supportRequestsUnsubscribe = supportRequestsCollection().onSnapshot(snap => {
    ...
    snap.docs.forEach(doc => {
      const data = doc.data() || {};
      if (data.type === "ABSENCE") allAbsences.push({ id: doc.id, ...data });
      if (Number(data.examEndMs || 0) >= now) rows.push({ id: doc.id, ...data });
    });
    supportAbsenceRows = allAbsences;
    updateSupportStats();
...
function getAttendanceKey(){
  if (!currentHall || !currentPeriod) return "";
  return `attendance.${currentHall}.${currentPeriod.start.getTime()}`;
}
function getStoredAttendanceReport(key=getAttendanceKey()){
  if (!key) return [];
  try { return JSON.parse(localStorage.getItem(key) || "[]") || []; } catch { return []; }
}
function setStoredAttendanceReport(rows, key=getAttendanceKey()){
  if (!key) return;
  localStorage.setItem(key, JSON.stringify(Array.isArray(rows) ? rows : []));
}
function isAttendanceSubmitted(){
  return getStoredAttendanceReport().length > 0 || localStorage.getItem(getAttendanceKey()+".submitted") === "1";
}
function absenceTotalForCurrentHall(){
  const stored = getStoredAttendanceReport();
  return stored.reduce((sum, r) => sum + (Number(r.absenceCount) || 0), 0);
}
function updateAbsenceCard(){
  const card = document.getElementById("absenceCard");
  const value = document.getElementById("absenceTotal");
  if (!card || !value) return;
  if (!currentHall || !currentPeriod) { card.classList.add("hidden"); return; }
  const total = absenceTotalForCurrentHall();
  value.textContent = toArabicDigits(total);
  card.classList.remove("hidden");
  card.classList.toggle("has-absence", total > 0);
}
function buildAttendanceRows(){
  return activePeriodExams.map((e, idx) => ({
    idx,
    courseName: e.courseName,
    courseCode: e.courseCode,
    section: e.section,
    sectionLabel: formatSectionLabel(e.section),
    students: e.students
  }));
}
function openAttendanceModal(){
  if (!currentHall || !currentPeriod || !activePeriodExams.length || isAttendanceSubmitted()) return;
  const modal = document.getElementById("attendanceModal");
  const body = document.getElementById("attendanceTableBody");
  if (!modal || !body) return;
  const rows = buildAttendanceRows();
  body.innerHTML = rows.map(r => `<tr data-index="${r.idx}">
    <td>${escapeHtml(r.courseName)}</td>
    <td>${escapeHtml(r.courseCode)}</td>
    <td>${escapeHtml(r.sectionLabel)}</td>
    <td><input class="attendance-input" type="number" min="0" max="${Number(r.students)||999}" value="0" inputmode="numeric" /></td>
  </tr>`).join("");
...
async function submitAttendanceReport(){
  if (!currentHall || !currentPeriod || !supportRequestsCollection()) return alert("لا يمكن إرسال الغياب بدون اتصال Firestore.");
  const rowsBase = buildAttendanceRows();
  const trs = [...document.querySelectorAll("#attendanceTableBody tr")];
  const reportRows = trs.map(tr => {
    const idx = Number(tr.dataset.index);
    const base = rowsBase.find(r => r.idx === idx) || rowsBase[idx] || {};
    const n = Number(tr.querySelector(".attendance-input")?.value || 0);
    return { ...base, absenceCount: Number.isFinite(n) && n > 0 ? Math.floor(n) : 0 };
  });
  setStoredAttendanceReport(reportRows);
  localStorage.setItem(getAttendanceKey()+".submitted", "1");
  const positives = reportRows.filter(r => Number(r.absenceCount) > 0);
  try {
    await Promise.all(positives.map(r => supportRequestsCollection().add({
      hall: currentHall,
      type: "ABSENCE",
      absenceCount: Number(r.absenceCount) || 0,
      courseName: r.courseName || "",
      courseCode: r.courseCode || "",
      section: r.section || "",
      sectionLabel: r.sectionLabel || formatSectionLabel(r.section || ""),
      createdAtMs: Date.now(),
      examStartMs: currentPeriod.start.getTime(),
      examEndMs: currentPeriod.end.getTime(),
      periodText: `${currentPeriod.startText} - ${currentPeriod.endText}`,
      status: "pending",
      acknowledged: false,
    ...
    updateAbsenceCard();
    showAppToast("تم إرسال بيانات الغياب.", 3000);
  } catch (err) {
    console.error(err);
    alert("تعذر إرسال بيانات الغياب. تحقق من قواعد Firestore.");
  }
}
function checkAttendancePrompt(){
  if (!currentPeriod || !currentHall || endHandled || attendancePromptShown || isAttendanceSubmitted()) return;
  const now = new Date();
  if (now < currentPeriod.start) return;
  const trigger = new Date(currentPeriod.start.getTime() + getAttendanceMinutes() * 60000);
  if (now >= trigger) {
    attendancePromptShown = true;
    openAttendanceModal();
  }
...
function initAttendanceModal(){
  ...
  document.getElementById("attendanceCancelBtn")?.addEventListener("click", () => {
    closeAttendanceModal();
...
function renderHall(hall){
  ...
  document.getElementById("absenceCard")?.classList.add("hidden");
...
function initTermSettings(){
  ...
  const attendanceMinutesInput = document.getElementById("attendanceMinutesInput");
  ...
  if (attendanceMinutesInput) attendanceMinutesInput.value = String(getAttendanceMinutes());
  ...
  saveBtn.addEventListener("click", () => {
    ...
    const attendanceMinutes = attendanceMinutesInput ? Number(attendanceMinutesInput.value) : getAttendanceMinutes();
    if (!Number.isFinite(attendanceMinutes) || attendanceMinutes < 0) return alert("يرجى إدخال وقت تسجيل الغياب بالدقائق بصورة صحيحة.");
    localStorage.setItem(STORE_KEYS.attendanceMinutes, String(attendanceMinutes));
    localStorage.setItem(STORE_KEYS.semester, sem);
    localStorage.setItem(STORE_KEYS.academicYear, ay);
    saveCloudSettings({ semester: sem, academicYear: ay, attendanceMinutes: attendanceMinutes });
    setTitles();
