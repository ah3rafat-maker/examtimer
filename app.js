...
function initCloudSync(){
  ...
  try {
    ...
    firestoreDoc(FIRESTORE_DOCS.settings).onSnapshot(snap => {
      ...
      if (!snap.exists) saveCloudSettings({ password: DEFAULT_PASSWORD, supportCode: localStorage.getItem(STORE_KEYS.supportCode) || "2026", attendanceTime: localStorage.getItem(STORE_KEYS.attendanceTime) || "30", semester: localStorage.getItem(STORE_KEYS.semester) || "ربيع", academicYear: localStorage.getItem(STORE_KEYS.academicYear) || "2025 - 2026", createdAt: Date.now() });
...
async function submitAttendanceReport(){
  ...
  try {
    ...
    showAppToast(total > 0 ? "تم إرسال بيانات الغياب" : "تم تسجيل عدم وجود غياب");
  } catch (err) {
    console.error(err);
    attendanceSubmitInProgress = false;
    if (btn) btn.disabled = false;
...
async function submitEarlyFinish(){
  ...
  try {
    if (supportRequestsCollection()) await supportRequestsCollection().add({
      hall: currentHall,
      type: "انتهاء اختبار",
      kind: "earlyFinish",
      message: msg,
  ...
  set.add(sectionExamKey(e));
  saveFinishedSectionsSet(set);
  closeEarlyFinishModal();
  showAppToast("تم إنهاء اختبار الشعبة");
