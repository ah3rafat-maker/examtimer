...
function handleForcedHallRelease(reason=""){
  const releasedHall = currentHall || "";
  ...
  showAppToast(releasedHall ? `تم تحرير القاعة ${releasedHall} من قبل المسؤول.` : "تم تحرير هذه القاعة من قبل المسؤول.", 2500);
  setTimeout(() => goHome(false), 900);
}
...
function showHallLockedState(hall, lockData={}){
  ...
  if (empty) {
    empty.classList.remove("hidden");
    empty.innerHTML = `<h2>القاعة ${escapeHtml(hall)} مفتوحة حاليًا على جهاز آخر.</h2><button class="secondary-btn" type="button" onclick="location.reload()">تحديث</button>`;
  }
}
...
function renderHallLocksList(docs){
  ...
  box.querySelectorAll("[data-release-lock]").forEach(btn => btn.addEventListener("click", async () => {
    ...
    try {
      ...
      await ref.set({ forceClose:true, releasedAtMs:Date.now(), releasedBy:adminUserEmail || PRIMARY_ADMIN_EMAIL }, { merge:true });
...
function initHallLockAdmin(){
  ...
  document.getElementById("releaseAllHallLocksBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      await Promise.all(snap.docs.map(d=>d.ref.set({ forceClose:true, releasedAtMs:Date.now(), releasedBy:adminUserEmail || PRIMARY_ADMIN_EMAIL }, { merge:true })));
      setTimeout(async () => {
        try {
