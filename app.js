...
function shouldApplyForceRelease(settings){
  ...
  const openedAt = Number(currentHallLockOpenedAt || 0);
  const allAt = Number(settings.lastForceCloseAllAtMs || 0);
  const oneAt = Number(settings.lastForceClosedAtMs || 0);
  const oneId = String(settings.lastForceClosedLockId || "");
  const oneHall = String(settings.lastForceClosedHall || "");
  const hasActiveHallPage = !!(currentHall || currentPeriod || currentHallLockId);
  if (!hasActiveHallPage) return false;
  if (allAt && (!openedAt || allAt >= openedAt - 1000)) return true;
  if (currentHallLockId && oneId && oneId === currentHallLockId && oneAt && (!openedAt || oneAt >= openedAt - 1000)) return true;
  if (currentHall && oneHall && oneHall === currentHall && oneAt && (!openedAt || oneAt >= openedAt - 1000)) return true;
  return false;
}
function applyForceReleaseFromSettings(settings){
  if (shouldApplyForceRelease(settings)) handleForcedHallRelease("settings-force");
}
...
function renderHallLocksList(docs){
  ...
  if (!box) return;
  const rows = docs.map(doc => ({ id: doc.id, ...(doc.data ? doc.data() : doc) }))
  ...
  box.innerHTML = `<table class="admin-locks-table"><thead><tr><th>القاعة</th><th>الفترة</th><th>وقت الفتح</th><th>إجراء</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${escapeHtml(r.hall||"")}</td><td>${escapeHtml(r.periodText||"")}</td><td>${r.openedAtMs ? escapeHtml(toArabicDigits(new Intl.DateTimeFormat("ar-OM-u-nu-latn-ca-gregory", {hour:"2-digit",minute:"2-digit"}).format(new Date(r.openedAtMs)))) : ""}</td><td><button class="danger-btn small-danger" data-release-lock="${escapeHtml(r.id)}" type="button">تحرير القاعة</button></td></tr>`).join("")}</tbody></table>`;
  box.querySelectorAll("[data-release-lock]").forEach(btn => btn.addEventListener("click", async () => {
    ...
    try {
      ...
      const data = snap.exists ? (snap.data() || {}) : {};
      const stamp = Date.now();
      await ref.set({ forceClose:true, releasedAtMs:stamp, releasedBy:adminUserEmail || PRIMARY_ADMIN_EMAIL }, { merge:true });
      await saveCloudSettings({
        lastForceClosedLockId: btn.dataset.releaseLock,
        lastForceClosedHall: data.hall || "",
        lastForceClosedAtMs: stamp
      });
      setTimeout(() => ref.delete().catch(()=>{}), 9000);
...
function initHallLockAdmin(){
  ...
  document.getElementById("releaseAllHallLocksBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      await saveCloudSettings({ lastForceCloseAllAtMs: stamp });
      setTimeout(async () => {
        try {
...
function updateTimer(){
  ...
  if (!startHandled) {
    startHandled = true;
    playBeep(getBeepDurationSetting("start"));
  }
