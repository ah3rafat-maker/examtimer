...
function renderSupportPeriodStats(todayExams){
  ...
  box.innerHTML = entries.map(([period, exams], idx) => {
    const halls = unique(exams.map(e => e.hall)).sort((a,b)=>a.localeCompare(b,"ar",{numeric:true}));
    ...
    const hallDetails = halls.map(hall => {
      const hallExams = exams.filter(e => e.hall === hall);
      return `<div class="support-hall-stat"><strong>${escapeHtml(hall)}</strong><span>${toArabicDigits(uniqueCourseCount(hallExams))} مقرر</span><span>${toArabicDigits(hallExams.length)} شعبة</span><span>${toArabicDigits(sumStudents(hallExams))} طالب</span></div>`;
    }).join("");
    return `<section class="support-period-block">
      <h4>الفترة ${toArabicDigits(idx + 1)} <small>${escapeHtml(period)}</small></h4>
      <div class="support-period-summary">
      ...
      <div class="support-halls-grid">${hallDetails}</div>
    </section>`;
  }).join("");
}
