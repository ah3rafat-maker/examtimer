...
function initCloudSync(){
  ...
  try {
    ...
    firestoreDoc(FIRESTORE_DOCS.settings).onSnapshot(snap => {
      ...
      if (!snap.exists) saveCloudSettings({ password: DEFAULT_PASSWORD, supportCode: localStorage.getItem(STORE_KEYS.supportCode) || "2026", attendanceTime: localStorage.getItem(STORE_KEYS.attendanceTime) || "30", supportChairName: localStorage.getItem(STORE_KEYS.supportChairName) || "", supportChairTitle: localStorage.getItem(STORE_KEYS.supportChairTitle) || "", semester: localStorage.getItem(STORE_KEYS.semester) || "ربيع", academicYear: localStorage.getItem(STORE_KEYS.academicYear) || "2025 - 2026", createdAt: Date.now() });
...
function archiveExpiredSupportRequests(snapshot, now = Date.now()){
  ...
  snapshot.docs.forEach(doc => {
    ...
    if (end && end < now) {
      supportRequestsCollection().doc(doc.id).delete().catch(() => {
        supportRequestsCollection().doc(doc.id).set({ archived:true, status:"archived" }, { merge:true }).catch(()=>{});
      });
    }
  });
}
...
function openAttendanceModalIfNeeded(){
  if (!currentHall || !currentPeriod || !activePeriodExams.length) return;
...
async function submitAttendanceReport(){
  ...
  try {
    for (const req of batch) await supportRequestsCollection().add(req);
    if (key) { localStorage.setItem(key, "1"); localStorage.setItem(key + ".total", String(total)); }
    attendanceSubmitted = true;
...
function renderHall(hall){
  ...
  attendanceSubmitted = !!(attendanceKey && localStorage.getItem(attendanceKey) === "1");
  attendanceModalShown = false;
...
function initTermSettings(){
  ...
  saveBtn.addEventListener("click", () => {
    ...
    saveCloudSettings({ semester: sem, academicYear: ay, attendanceTime });
    setTitles();
...
function buildSupportReportHtml(requests=[]){
  ...
  const today = getTodayExamsForSupport();
  const absenceReqs = requests.filter(r=>r.kind === "absence");
  const earlyReqs = requests.filter(r=>r.kind === "earlyFinish");
  const activeReqs = requests.filter(r=>r.kind !== "absence" && r.kind !== "earlyFinish");
  const absenceTotal = absenceReqs.reduce((s,r)=>s+(Number(r.absenceCount)||0),0);
  const todayStudents = sumStudents(today);
  const summaryRows = [
    ["عدد لجان اليوم", toArabicDigits(computeCommitteeCount(today))],
    ["عدد القاعات المستخدمة", toArabicDigits(unique(today.map(e=>e.hall)).length)],
    ["عدد المقررات", toArabicDigits(uniqueCourseCount(today))],
    ["عدد الشعب", toArabicDigits(today.length)],
    ["عدد الطلبة", toArabicDigits(todayStudents)],
    ["عدد الغياب", toArabicDigits(absenceTotal)],
    ["نسبة الغياب", toArabicDigits(todayStudents ? ((absenceTotal/todayStudents)*100).toFixed(2) : "0.00") + "%"],
    ["عدد الاستدعاءات", toArabicDigits(activeReqs.length)],
    ["عدد إشعارات إنهاء الاختبارات", toArabicDigits(earlyReqs.length)]
  ];
  const periodMap = new Map();
  today.forEach(e => { const k = periodKeyForExam(e); if (!periodMap.has(k)) periodMap.set(k, []); periodMap.get(k).push(e); });
  ...
  const hallRows = today.slice().sort((a,b)=>String(a.hall).localeCompare(String(b.hall),'ar',{numeric:true})).map(e=>[
  ...
  const requestRows = requests.slice().sort((a,b)=>(a.createdAtMs||0)-(b.createdAtMs||0)).map(r=>[
  ...
  let sections = "";
  if (selected.summary) sections += `<h2>الملخص الإحصائي</h2>${tableHtml(["البيان","القيمة"], summaryRows.map(r=>[escapeHtml(r[0]), r[1]]))}`;
  ...
  if (selected.absence) sections += `<h2>إحصائية الغياب</h2>${tableHtml(["الوقت","القاعة","النوع","التفاصيل"], requestRows.filter(r=>/غياب/.test(r[2]+r[3])))}`;
  if (selected.requests) sections += `<h2>سجل الاستدعاءات</h2>${tableHtml(["الوقت","القاعة","النوع","التفاصيل"], requestRows)}`;
  ...
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>تقرير لجنة دعم ومراقبة الامتحانات</title><style>
    @page{size:A4;margin:14mm}body{font-family:Arial,Tahoma,sans-serif;color:#0B2E6B;line-height:1.55}.print-header{text-align:center;border-bottom:2px solid #0B2E6B;padding-bottom:12px;margin-bottom:18px}.print-logo{max-height:78px;margin-bottom:8px}.print-header h1{font-size:24px;margin:4px 0}.print-header h2{font-size:18px;margin:3px 0;color:#123}.print-section h2,h2{font-size:18px;margin:18px 0 8px;color:#0B2E6B}.print-table{width:100%;border-collapse:collapse;margin-bottom:12px;font-size:12px}.print-table th,.print-table td{border:1px solid #B8C6D9;padding:6px;text-align:center;vertical-align:middle}.print-table th{background:#EAF2FF}.signature-block{margin-top:36px;text-align:left;color:#111}.report-date{text-align:left;color:#555;font-size:12px;margin-top:8px}@media print{button{display:none}}
  </style></head><body><div class="print-header">${logo}<h1>تقرير لجنة دعم ومراقبة الامتحانات</h1><h2>الامتحانات النهائية للفصل الدراسي (${escapeHtml(title.semester)})</h2><h2>العام الأكاديمي (${escapeHtml(title.academicYear)})</h2><div class="report-date">تاريخ إنشاء التقرير: ${escapeHtml(toArabicDigits(new Intl.DateTimeFormat("ar-OM-u-nu-latn-ca-gregory", {dateStyle:"medium", timeStyle:"short"}).format(new Date())))}</div></div>${sections}${signature}<script>setTimeout(()=>window.print(),400)</script></body></html>`;
}
...
function startSupportWakeMode(){
  ...
  supportWakeInterval = setInterval(updateSupportWakeLock, 30000);
}
