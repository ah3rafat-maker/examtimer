...
function shouldApplyForceRelease(settings){
  ...
  // احتياطي للجلسات القديمة التي لا تملك currentHallLockId صحيحًا
  if (currentHall && oneHall && oneHall === currentHall && oneAt && !currentHallLockId) return true;
  return false;
}
function applyForceReleaseFromSettings(settings){
  if (!settings) return;
  const serial = String(settings.forceReleaseSerial || settings.lastForceCloseAllAtMs || "");
  if (serial && serial === lastForceReleaseHandledSerial) return;
  if (shouldApplyForceRelease(settings)) {
    lastForceReleaseHandledSerial = serial || String(Date.now());
    handleForcedHallRelease("settings-force");
  }
}
